...
async function sendVerifyMessage(ctx) {
  const caption =
    'Silahkan follow *3 channel berikut* terlebih dahulu untuk menggunakan bot ini:\n\n';
...
let text = `👋 Halo <b>${username}</b>\n`;
  text += '<blockquote>Selamat datang di bot Pemersatu Bangsa. Saya di sini bertugas untuk memberi kalian semua bahan Video yang gacor dan free ya pastinya.</blockquote>\n\n';
  ...
  text += `\n<blockquote>Halaman ${page}/${totalPages}</blockquote>`;
...
bot.action('verify_follow', async (ctx) => {
  ...
  const teksSukses =
    ...
    'Pencet tombol Start di bawah ini untuk mulai.';
...
bot.command('stats', async (ctx) => {
  ...
  for (const [id] of users) {
    const sub = await isSubscribed(ctx, id);
    if (sub) subscribedCount++;
  }

  ctx.reply(
    `📊 Statistik Bot:\n` +
      `Total user terdaftar: ${totalUsers}\n` +
      `Masih follow channel: ${subscribedCount}\n` +
      `Belum follow / keluar: ${totalUsers - subscribedCount}`
  );
});
// =====================
// BROADCASH
